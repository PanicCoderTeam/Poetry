...
func TestPoetryRepository_DescribePeotryInfo(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for {
				got, got1, err := r.DescribePeotryInfo(tt.args.ctx, []string{}, []string{}, []string{}, []string{}, []string{}, []int64{}, limit, offset)
				offset += limit
				if offset >= int(got) {
					break
				}
				...
				for _, poetry := range got1 {
					...
					poetry.ParagraphsTradition = utils.ConvertChinsesSimplified2T(poetry.Paragraphs)
					poetry.Paragraphs = utils.ConvertChinsesTraditional2S(poetry.Paragraphs)
